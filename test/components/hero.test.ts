...
describe('TitleCardHero', () => {
  it('renders the title-card composition (zone name, tagline, swoosh, version)', async () => {
    ...
    expect(html).toContain('Open-Source Sonic Engine');      // tagline (moved up into the blue)
    expect(html).toContain('data-tc="red"');                 // red left block (toothed swoosh)
    expect(html).toContain('data-tc="act"');                 // version slot present
    expect(html).toContain('Latest Version');                // version plate label
